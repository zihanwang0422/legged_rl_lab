...
def handstand_feet_air_time(env: ManagerBasedRLEnv, sensor_cfg: SceneEntityCfg, threshold: float) -> torch.Tensor:
    ...
    # Check if ALL specified feet are in the air (not in contact)
    all_feet_in_air = torch.all(~contact_sensor.data.net_forces_w_history[:, :, sensor_cfg.body_ids, :].norm(dim=-1).max(dim=1)[0] > 1.0, dim=1)
    
    # Only reward if all feet were in air and threshold is met
