...
class UnitreeGo1FootstandEnvCfg(LocomotionVelocityRoughEnvCfg):
    def __post_init__(self):
        ...
        if hand_stand_type == "rear":
            ...
            # Symmetry settings for rear footstand
            rear_joint_symmetry_weight = -1.0
            rear_action_symmetry_weight = -0.5
            # Front leg penalties to prevent 3-leg landing
            front_undesired_contact_weight = -2.0  # Penalize front foot contact
            front_joint_symmetry_weight = -1.0     # Ensure front legs move symmetrically
            front_action_symmetry_weight = -0.5    # Ensure front actions are symmetric
            # self.rewards.base_ang_vel_x_l2.weight = -0.005
            # self.rewards.base_ang_vel_x_l2.params["asset_cfg"] = SceneEntityCfg("robot")
        ...
        # 5. Rear Leg Symmetry Rewards - ensure rear legs are symmetric
        self.rewards.joint_symmetry_l2.weight = rear_joint_symmetry_weight
        self.rewards.joint_symmetry_l2.params["mirror_joints"] = [
            ["RL_.*_joint", "RR_.*_joint"],  # Rear left-right symmetry
        ]
        
        self.rewards.action_symmetry_l2.weight = rear_action_symmetry_weight
        self.rewards.action_symmetry_l2.params["mirror_joints"] = [
            ["RL_.*_joint", "RR_.*_joint"],  # Rear left-right symmetry
        ]
        
        # 6. Front Leg Penalties - prevent single front leg contact (3-leg landing)
        # Add a new undesired_contacts term specifically for front feet
        self.rewards.front_feet_contact_penalty = RewTerm(
            func=mdp.undesired_contacts,
            weight=front_undesired_contact_weight,
            params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=["FL_foot", "FR_foot"]), "threshold": 1.0},
        )
        
        # 7. Front Leg Symmetry - ensure both front legs move together (not one up, one down)
        self.rewards.front_joint_symmetry_l2 = RewTerm(
            func=mdp.joint_mirror,
            weight=front_joint_symmetry_weight,
            params={
                "asset_cfg": SceneEntityCfg("robot"),
                "mirror_joints": [["FL_.*_joint", "FR_.*_joint"]],  # Front left-right symmetry
            },
        )
        
        self.rewards.front_action_symmetry_l2 = RewTerm(
            func=mdp.action_mirror,
            weight=front_action_symmetry_weight,
            params={
                "asset_cfg": SceneEntityCfg("robot"),
                "mirror_joints": [["FL_.*_joint", "FR_.*_joint"]],  # Front left-right symmetry
            },
