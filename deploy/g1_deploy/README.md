...
在运动控制模式下，按下遥控器上的 Start 按钮，机器人将进入阻尼模式并下落，程序将退出。或者在终端中使用 `Ctrl+C` 关闭程序。

**注意：**

由于此示例部署不是稳定的控制程序，仅用于演示目的，请尽量不要在控制过程中干扰机器人。如果控制过程中发生任何意外情况，请及时退出控制以避免危险。


### 手柄控制 (Unitree 官方遥控器，通过机器人 DDS)
...
- **倾倒保护**：roll 或 pitch 超过 0.8 rad 自动停止发送指令
- **超时保护**：`lowstate` 超过 0.2s 未更新自动停止
