...
Usage:
    Simulation: python deploy/go2_deploy/sim2sim_walk.py
"""
...
class Sim2SimController:
    ...
    def __init__(self, config_path, model_name):
        ...
        obs_dim = 9 + 3 * self.num_joints  # 3(ang_vel)+3(grav)+3(cmd) + 3*n_joints
        self.obs_history = deque([np.zeros(obs_dim, dtype=np.float32)] * 5, maxlen=5)
        self.kp = c.kp_walk
        self.kd = c.kd_walk
        ...
        self.data.qpos[2] = getattr(c, 'init_height', 0.40) # 优先从配置读高度
        ...
        with mujoco.viewer.launch_passive(self.model, self.data) as viewer:
            ...
            while viewer.is_running():
                ...
                if motiontime % self.policy_decimation == 0:
                    ...
                    obs_arr = np.array(list(self.obs_history))  # (5, )
                    n = self.num_joints  # 12
                    ...
                    # Parse 12D action: position offsets
                    self.last_action = action.copy()
                        
                    # Position action: 12 dims (relative to default), scale
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='go2_rough.pt')
    parser.add_argument('--config', type=str, default='go2_walk.yaml')
    ...
    print("  RB + A  : Walk policy  (go2_walk)")
